...
class MS_PT_MolecularPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if psys.settings.mol_gravity_active:
            row = box.row()
            row.prop(psys.settings, "mol_gravity_strength", text="Strength")
            row.prop(psys.settings, "mol_gravity_theta", text="Accuracy")
            row = box.row()
            row.prop(psys.settings, "mol_gravity_softening", text="Softening")
            row = box.row()
            row.prop(psys.settings, "mol_gravity_initial_rotation", text="Init Rotation")
            row.prop(psys.settings, "mol_gravity_rotation_falloff", text="Falloff")

        ###   Initial Positions CSV   ###
        box = layout.box()
        row = box.row()
        row.label(text="Initial Positions :")
        row = box.row()
        row.prop(psys.settings, "mol_initial_csv", text="CSV File")

        # Scale options
        row = box.row()
        row.prop(psys.settings, "mol_csv_scale_mode", text="Scale Mode")
        row = box.row()
        row.prop(psys.settings, "mol_csv_min_scale", text="Min Scale")
        row.prop(psys.settings, "mol_csv_scale_multiplier", text="Multiplier")
